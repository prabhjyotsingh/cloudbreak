...
public class AwsNetworkConnector implements NetworkConnector {
    ...
    @Override
    public CreatedCloudNetwork createNetworkWithSubnets(String envName, CloudCredential cloudCredential, Region region, String vpcCidr,
            Set<String> subnetCidrs) {
        AwsCredentialView credentialView = new AwsCredentialView(cloudCredential);
        AmazonCloudFormationRetryClient cloudFormationRetryClient = awsClient.createCloudFormationRetryClient(credentialView, region.value());
        String cloudFormationTemplate = awsNetworkCfTemplateProvider.provide(vpcCidr, subnetCidrs);
        ...
        AmazonCloudFormationClient cfClient = awsClient.createCloudFormationClient(credentialView, region.value());
        PollTask<Boolean> pollTask = awsPollTaskFactory.newAwsCreateNetworkStatusCheckerTask(cfClient, CREATE_COMPLETE, CREATE_FAILED, ERROR_STATUSES, envName);
        try {
        ...
        Set<String> subnetIds = getCreatedSubnet(output, subnetCidrs.size());
    ...
    private Set<String> getCreatedSubnet(Map<String, String> output, int numberOfSubnets) {
        Set<String> subnetIds = new HashSet<>();
        for (int i = 0; i < numberOfSubnets; i++) {
            String expectedSubnetName = CREATED_SUBNET + i;
            if (output.containsKey(expectedSubnetName)) {
                subnetIds.add(output.get(expectedSubnetName));
            } else {
