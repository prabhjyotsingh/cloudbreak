...
public class CloudNetworkCreationHandlerTest {
    ...
    public static final String VARIANT = "v4";
    ...
    public void testAcceptShouldCallTheCloudPlatformConnectorsWithTheProperParameters() {
        ...
        when(networkConnector.createNetworkWithSubnets(request.getEnvName(), request.getCloudCredential(), Region.region(request.getRegion()),
                request.getNetworkCidr(), request.getSubnetCidrs())).thenReturn(createdCloudNetwork);
        ...
        verify(networkConnector).createNetworkWithSubnets(request.getEnvName(), request.getCloudCredential(), Region.region(request.getRegion()),
    ...
    private Event<CloudNetworkCreationRequest> createCloudNetworkRequestEvent() {
        ...
        CloudNetworkCreationRequest request = new CloudNetworkCreationRequest(ENV_NAME, extendedCloudCredential, extendedCloudCredential, VARIANT,
                REGION, NETWORK_CIDR, SUBNET_CIDRS);
